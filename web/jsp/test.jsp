...
%>
<link href="../static/pagination.css" rel="stylesheet" type="text/css"/>
...
<script type="text/javascript">
    ...
    function handlePaginationClick(new_page_index, pagination_container) {
        $("#stuForm").attr("action", "<%=context %>/users?currentPage=" + (new_page_index+1));
        $("#stuForm").submit();
        return false;
    }
    ...
    $(function(){
        $("#News-Pagination").pagination(${pager.totalNums}, {
            items_per_page:${pager.showNums}, // 每页显示多少条记录
...
<div style="margin-left: 100px; margin-top: 100px;">
    ...
    <div>
        <form action="<%=context %>/users"   id="stuForm"  method="post">
            姓名
    ...
    <br>
    <!-- 后台返回结果为空 -->
    <c:if test="${fn:length(pager.dataList) eq 0 }">
        <span>查询的结果不存在</span>
    </c:if>

    <!-- 后台返回结果不为空 -->
    <c:if test="${fn:length(pager.dataList) gt 0 }">
        <table border="1px" cellspacing="0px"
            ...
            <c:forEach items="${pager.dataList }" var="student">
                <tr>
